    ...
    public class SqlServerInfoPersistence : IDbInfoPersistence
    {
        ...
        public IList<ColumnInfo> SelectColumns(string dataBase, string connectionString, string table)
        {
            string sql = "select t.*, p.value [Description] from"
                        + " ("
                        + " select a.name Name, c.name as DataType,case when a.is_nullable = 0 then 0 else 1 end as [IsNull],a.max_length[Length], a.column_id column_id"
                        ...
                        + " ) t"
                        + " left join sys.extended_properties p on p.minor_id = t.column_id and p.name = 'MS_Description'"
