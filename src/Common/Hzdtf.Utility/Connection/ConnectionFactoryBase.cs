...
namespace Hzdtf.Utility.Connection
{
        ...
        where ConnectionWrapInfoT : ConnectionWrapInfo<ConnectionInfoT>
    {
        ...
        public virtual ConnectionT CreateAndOpen(ConnectionWrapInfoT connectionWrap = null)
        {
            ...
            {
                var connString = GetConnectionStringByConfigName(connectionWrap.ConnectionStringAppConfigName);
                if (!string.IsNullOrWhiteSpace(connString))
                {
                    if (ConfigConnectionEncryption())
                    {
        ...
        protected abstract string GetConnectionStringByConfigName(string configName);

        /// <summary>
        /// 配置连接是否加密，默认为否
        /// </summary>
        /// <returns>配置连接是否加密</returns>
        protected virtual bool ConfigConnectionEncryption() => false;
