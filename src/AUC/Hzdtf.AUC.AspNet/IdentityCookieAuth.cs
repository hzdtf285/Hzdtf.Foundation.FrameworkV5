...
namespace Hzdtf.AUC.AspNet
{
        ...
        where UserT : BasicUserInfo<IdT>
    {
        ...
        protected override void SignIn(ClaimsPrincipal principal)
        {
            ...
            if (!string.IsNullOrWhiteSpace(App.CurrConfig["Cookies:Expires"]))
            {
                prop = new AuthenticationProperties()
                {
                    ExpiresUtc = DateTime.UtcNow.AddMinutes(Convert.ToInt32(App.CurrConfig["Cookies:Expires"])),
                    IsPersistent = true
                };
            }
            httpContext.HttpContext.SignInAsync(principal, prop);
        }
